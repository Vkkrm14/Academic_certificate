...
export default function HODPage() {
    ...
    const checkHODRole = async () => {
      ...
      try {
        const result = await readContract({
          ...
          method: "function isHOD(address) view returns (bool)",
          params: [account.address],
        });
        setIsHOD(result as boolean);
      } catch (error) {
        console.error("Error checking HOD role:", error);
        setIsHOD(false);
      }
    };
  ...
  // Load current pending approvals (students pending HOD approval)
  const loadCurrentPending = useCallback(async () => {
    if (!isHOD || isLoadingRef.current || dataLoadedRef.current.current) return;
  ...
  }, [isHOD, certificateContract]);

  // Load all pending approvals at all levels
  const loadAllLevelPending = useCallback(async () => {
    if (!isHOD || isLoadingRef.current || dataLoadedRef.current.allLevels)
      return;
  ...
  const loadIssuedCertificates = useCallback(async () => {
    if (!isHOD || isLoadingRef.current || dataLoadedRef.current.issued) return;
  ...
  const handleApprove = async (regNo: string) => {
    if (!account || !isHOD) {
      alert("Only HOD accounts can approve students");
      return;
    }
    ...
    try {
      ...
      const transaction = prepareContractCall({
        contract: certificateContract,
        method: "function approveByHOD(string memory regNo) public",
        params: [regNo],
      });
      ...
      sendTransaction(transaction, {
        onSuccess: () => {
          alert(
            `Student ${regNo} approved successfully! Moving to Dean level.`,
          );
  ...
  const handleReject = async () => {
    if (!account || !isHOD || !rejectRegNo || !rejectReason) {
    ...
    try {
      ...
      const transaction = prepareContractCall({
        contract: certificateContract,
        method:
          "function rejectByHOD(string memory regNo, string memory reason) public",
      ...
      <div className="container mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <Link
              href="/"
              className="text-green-200 hover:text-white mb-2 inline-block"
            >
              ← Back to Home
            </Link>
            <h1 className="text-4xl font-bold text-white">HOD Dashboard</h1>
            <p className="text-green-200">
              First level approval - Review and approve student applications
            </p>
          </div>
          ...
          <div className="text-center py-20">
            <div className="glass-effect rounded-lg p-8">
              ...
              <p className="text-gray-300">
                Please connect your wallet with HOD role to access this
                dashboard
              </p>
            </div>
          </div>
          ...
          <div className="text-center py-20">
            <div className="bg-red-500/20 backdrop-blur-md rounded-lg p-8 border border-red-500/50">
              ...
              <p className="text-red-300">
                Only accounts with HOD role can access this page
              </p>
          ...
          <>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 disabled:bg-gray-600 text-white px-4 py-3 rounded-lg font-semibold"
              >
                🔄 Refresh
              </button>
            </div>
            ...
            {activeTab === "current" && (
              <div className="glass-effect rounded-lg p-8">
                ...
                {isLoadingDetails ? (
                  ...
                  <div className="text-center py-12">
                    <p className="text-gray-300 text-xl">
                      ✅ No pending approvals at HOD level
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-purple-400 font-mono text-xs">
                              {student.departmentVerifier.slice(0, 10)}...
                            </p>
                          </div>
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div className="bg-gray-900 rounded-lg p-8 max-w-md w-full mx-4 border border-red-500/50">
              ...
              <textarea
                ...
                placeholder="Enter reason for rejection (e.g., Incomplete documentation, CGPA below requirement)..."
